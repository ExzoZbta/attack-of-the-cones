...
class Pipeline{
    ...
    GLuint CompilePipeline(string pipelineName, string vertexShaderSourceFilename, string fragmentShaderSourceFilename){
        ...
        if (linkStatus == GL_FALSE) {
            ...
            glGetProgramInfoLog(mProgramObjectID, infoLogLength, null, strInfoLog.ptr);
            writeln("Linker failure: ", strInfoLog);
        }
    ...
    static Pipeline GetPipeline(string name) {
        // Create a new instance to hold the pipeline
        Pipeline p = new Pipeline();
        p.mPipelineName = name;
        
        // Make sure the pipeline exists
        p.mProgramObjectID = PipelineCheckValidName(name);
        
        return p;
    }

    /// Constructor without initializing shaders
    this() {
        // This constructor is used for GetPipeline
    }

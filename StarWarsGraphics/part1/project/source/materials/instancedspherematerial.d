/// Material for instanced textured spheres
module instancedspherematerial;
...
/// Material for instanced textured spheres
class InstancedSphereMaterial : IMaterial {
    Texture mDiffuseMap;
    ...
    /// Constructor for instanced sphere material
    this(string pipelineName, InstancedSphereOnObjSurface objSurface) {
        super(pipelineName);
        ...
        if(diffusePath !is null && diffusePath.length > 0) {
            try {
                ...
                mHasDiffuseMap = true;
                writeln("Loaded diffuse texture for instanced spheres: ", diffusePath);
            } catch(Exception e) {
        ...
        if(specPath !is null && specPath.length > 0) {
            try {
                ...
                writeln("Loaded specular texture for instanced spheres: ", specPath);
            } catch(Exception e) {
        ...
        if(normalPath !is null && normalPath.length > 0) {
            try {
                ...
                mHasNormalMap = true;
                writeln("Loaded normal texture for instanced spheres: ", normalPath);
            } catch(Exception e) {
